...
from dataset_utils import boundary_modification
import cv2
...
def gt_augmentation(mask, p=0.3):
    if np.random.rand() > p:
        return mask  
...
class PolypObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, trainsize, mean=None, std=None, randomPeper=False,
        ...
        self.aug_transform = albumentations.Compose([
                albumentations.PadIfNeeded(min_height=int(trainsize*1.5), min_width=int(trainsize*1.5), border_mode=0, value=0, p=1.0),
                albumentations.ShiftScaleRotate(shift_limit=0.0, scale_limit=0.25, rotate_limit=15, border_mode=0, value=0, p=0.5),
                albumentations.HorizontalFlip(p=0.5),
                albumentations.VerticalFlip(p=0.5),
                albumentations.RandomCrop(height=trainsize, width=trainsize, p=1.0)
        ])
