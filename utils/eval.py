...
def eval(mask_path='./dataset',
         pred_path='./test_results',
         dataset_name=['COD10K', 'NC4K', 'CAMO', 'CHAMELEON']):
    all_results = {}
    global mask_root, pred_root

    for dataset in dataset_name:
        if dataset == "COD10K":
            mask_root = os.path.join(mask_path, "COD10K/Test/GT_Object/")
        elif dataset == "CAMO":
            mask_root = os.path.join(mask_path, "CAMO/GT_test/")
        elif dataset == "NC4K":
            mask_root = os.path.join(mask_path, "NC4K/GT/")
        elif dataset == "CHAMELEON":
            mask_root = os.path.join(mask_path, "CHAMELEON/GT/")
        else:
        ...
        print(dataset, ":", {k: f"{v:.4f}" for k, v in results.items()})
